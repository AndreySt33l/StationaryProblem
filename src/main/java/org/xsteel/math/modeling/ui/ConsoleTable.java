...
public class ConsoleTable{
    ...
    public void printTable(){
        ...
        sb.append("\n");
        //HEADERS
        builder(sb, padder, rowSeperator, headers);

        //BODY
        for(int i = 0; i < table.size(); i++){
            ArrayList<String> tempRow = table.get(i);
            //New row
            builder(sb, padder, rowSeperator, tempRow);
        }
        System.out.println(sb.toString());
    }

    private void builder(StringBuilder sb, String padder, String rowSeperator, ArrayList<String> headers) {
        sb.append("|");
        for(int i = 0; i < headers.size(); i++){
            sb.append(padder);
